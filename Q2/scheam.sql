...
const query = `SELECT TenantID FROM AptTenants GROUP BY TenantsID HAVING count(*)>1) C ON  Tenants.TenantID=C.TenantID`

/* - Write a SQL query to get a list of all buildings and the number of open requests (Requests in which
status equals 'Open'). */

 const query_2 = `SELECT Apartments.BuildingID,count(*) as 'Count' FROM Requests INNER JOIN Apartments
...
apartments in this building. */
 ...
 const query_3 = `SELECT AptID FROM Apartments WHERE BuildingID=11`